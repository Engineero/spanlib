...
#        return inputs
    ...
    #################################################################
    @_filldocs_
    def pca(self, force=False, optimize=False, **kwargs):
        """
        ...
        # Optimize?
        optimize = optimize or optimize is None and self.masked
        ...
        if not force and self._pca_raw_pc is not None and \
            ...
            if optimize:
                _optimec.optimec_pca(self.stacked_data, self._pca_raw_eof,
                    self._pca_raw_pc, default_missing_value)
            return
...
#            pdata = npy.asfortranarray(pdata)
        ...
        # Optimize
        if optimize :
            _optimec.optimec_pca(pdata, raw_eof, raw_pc, default_missing_value)
    ...
    @_filldocs_
    def pca_pc(self, scale=False, raw=False, format=True, optimize=False, **kwargs):
        ...
        if not optimize and self._pca_fmt_pc is not None:
        ...
        # Optimize
        if optimize:
            _optimec.optimec_pca(A.stacked_data, self._pca_raw_eof, self._pca_raw_pc,
    ...
    def pca_ec(self, xdata=None, xeof=None, scale=False, ev=None,
        ...
        optimize=False, replace=False, **kwargs):
        ...
        # Optimize
        if optimize or (optimize is None and
            (npy.isclose(raw_data, default_missing_value).any() or
                npy.isclose(raw_eof, default_missing_value).any())):
            _optimec.optimec_pca(raw_data, raw_eof, raw_ec, default_missing_value)
